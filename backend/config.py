...
class LLMConfig(BaseSettings):
    ...
    image_caption_model_id: str = Field(default='', alias='IMAGE_CAPTION_MODEL_ID')
    ...
    def effective_api_key(self) -> str:
        """优先使用 LLM_API_KEY；若为空则回退 DASHSCOPE_API_KEY。"""
...
class RetrievalConfig(BaseSettings):
    ...
    # 查询增强
    enable_mqe: bool = Field(default=True, alias='RETRIEVAL_ENABLE_MQE')
    enable_hyde: bool = Field(default=True, alias='RETRIEVAL_ENABLE_HYDE')
    mqe_query_count: int = Field(default=2, alias='RETRIEVAL_MQE_QUERY_COUNT')
