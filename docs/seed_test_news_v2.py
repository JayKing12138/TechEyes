...
async def seed_test_data():
    ...
    try:
        ...
        print(f"\n写入实体间关系...")
        for idx, relation in enumerate(entity_relations):
            print(f"[{idx + 1}/{len(entity_relations)}] {relation['from_entity']} -> {relation['to_entity']}")
            
            from_id = make_entity_id(relation["from_entity"], "company")
            to_id = make_entity_id(relation["to_entity"], "company")
        ...
        print(f"✅ 成功写入 {len(entity_relations)} 条实体关系！")
        ...
        print(f"📊 数据库中共有 {result[0]['count']} 条MENTIONS关系")
        
        result = neo4j_client.run_query("MATCH ()-[r:RELATES_TO]->() RETURN count(r) as count")
        print(f"📊 数据库中共有 {result[0]['count']} 条RELATES_TO关系")
